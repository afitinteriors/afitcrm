  ...
  return (
    <div className="space-y-5 p-4">
      <div className="flex items-center gap-3">
        <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full bg-[#14342a] text-xl font-semibold text-white">
          {(lead.customer_name || "?").charAt(0).toUpperCase()}
        </div>
        <div className="min-w-0">
          <p className="truncate text-base font-semibold text-slate-900">{lead.customer_name || "Unnamed lead"}</p>
      ...
      <dl className="space-y-3 text-sm">
        {lead.location && (
          <div>
            ...
            <dd className="mt-0.5 text-slate-900">{lead.location}</dd>
          </div>
        )}
        {lead.service_required && (
          <div>
            <dt className="text-xs font-medium uppercase tracking-wide text-slate-500">Service required</dt>
            <dd className="mt-0.5 text-slate-900">{lead.service_required}</dd>
          </div>
        )}
      </dl>
      ...
      <div className="grid grid-cols-2 gap-2 pt-1">
        <a
          ...
          title="Call"
          className="flex h-11 items-center justify-center gap-2 rounded-lg border border-slate-200 text-sm font-medium text-slate-700 active:bg-slate-100"
        >
        ...
        <a
          ...
          className="flex h-11 items-center justify-center gap-2 rounded-lg border border-slate-200 text-sm font-medium text-[#14342a] active:bg-slate-100"
        >
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-5 w-5">
      ...
      <Link
        href={`/leads/${lead.id}`}
        className="block rounded-lg bg-[#caa24a] py-2.5 text-center text-sm font-semibold text-[#14342a] active:bg-[#bb9440]"
      >
        View full lead
      </Link>
    </div>
