  ...
  if (!conversation) {
    return (
      <Card title="WhatsApp Conversation">
        <p className="text-sm text-slate-500">No WhatsApp conversation linked yet.</p>
      </Card>
    );
  }
  ...
  return (
    <Card title="WhatsApp Conversation">
      <div className="flex items-start justify-between gap-2">
        <p className="min-w-0 flex-1 truncate text-sm text-slate-700">{messageSnippet(conversation.lastMessage)}</p>
        <span
      ...
      {conversation.lastMessage && (
        <p className="mt-1 text-xs text-slate-400">{formatRelative(conversation.lastMessage.created_at)}</p>
      )}
      <Link
